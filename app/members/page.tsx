...
export default function MembersPage() {
  ...
  const [orgStructure, setOrgStructure] =
    useState<OrgStructure[]>(mockOrgStructure);
  ...
  const updateOrgStructureWithRealCounts = (
    employees: Employee[],
    ...
    const loadEmployees = async () => {
      try {
        ...
        if (data.success && data.members && data.members.length > 0) {
          ...
          const updatedOrgStructure = updateOrgStructureWithRealCounts(
            employees,
            mockOrgStructure
          );
          ...
          const sortedEmployees = [...mockEmployees].sort(
            (a, b) =>
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "employeeUpdated",
      handleEmployeeUpdateEvent as EventListener
    );
    ...
    try {
      ...
      if (result.success) {
        ...
        const newMember: Employee = {
          ...
          organizations: Array.isArray(memberData.organizations)
        ...
        const updatedEmployees = [...employees, newMember].sort(
          (a, b) =>
            new Date(b.joinDate).getTime() - new Date(a.joinDate).getTime()
        );
      ...
      toast.error(
        "구성원 추가 중 오류가 발생했습니다: " + (error as Error).message
      );
    }
  };
