...
export function FixedWorkForm({ formData, setFormData }) {
  ...
  return (
    <div className="space-y-8">
        ...
        <div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => openTimePicker("workTimeStart")}
              ...
              >
                {formData.workTimeStart || "09:00"}
              </button>
              <span className="text-gray-400">-</span>
              <button
                type="button"
                onClick={() => openTimePicker("workTimeEnd")}
                className="px-3 py-2 text-sm bg-white/60 backdrop-blur-sm border border-gray-200/50 rounded-lg hover:bg-white/80 transition-colors"
              >
                {formData.workTimeEnd || "18:00"}
              </button>
            </div>
          </div>
      ...
      {/* Time Picker Modals */}
      {showTimePicker.workTimeStart && (
        <SelectTime
          onTimeSelect={(time) => handleTimeSelect("workTimeStart", time)}
          onClose={() => closeTimePicker("workTimeStart")}
        />
      )}
      {showTimePicker.workTimeEnd && (
        <SelectTime
          onTimeSelect={(time) => handleTimeSelect("workTimeEnd", time)}
          onClose={() => closeTimePicker("workTimeEnd")}
        />
      )}
